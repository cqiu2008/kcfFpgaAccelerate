...
#ifndef HLS_FFT_PIPELINED_STREAM_HPP
#define HLS_FFT_PIPELINED_STREAM_HPP
...
INLINE void fft_core(
    complex<FFT_INPUT_T> xn[FFT_CHANNELS][FFT_LENGTH_QC],
    complex<FFT_OUTPUT_T> xk[FFT_CHANNELS][FFT_LENGTH_QC],
...
#pragma HLS inline
    ...
    if ( has_direction || has_scaling_sch )
        for (int i = 0; i < FFT_LENGTH_QC; ++i)
//    	for (int i = 0; i < fftRows*(1<<config_ch->getNfft()); ++i)
        {
...
//////////////////////////////////////////////
    ...
    for(int rowCnt=0;rowCnt<fftRows;rowCnt++) {
					...
					for (int c = 0; c < FFT_CHANNELS; ++c)
					{
						...
						{
							complex<FFT_INPUT_T> din = xn[c][i+rowCnt*samples];
...
>
INLINE void fft_core(
    complex<FFT_INPUT_T> xn[FFT_LENGTH_QC],
    complex<FFT_OUTPUT_T> xk[FFT_LENGTH_QC],
    ip_fft::status_t<CONFIG_T>* status,
    ip_fft::config_t<CONFIG_T>* config_ch,
	int fftRows)
{
...
#pragma HLS inline
...
#else
    complex<FFT_INPUT_T> xn_multi_chan [1][FFT_LENGTH_QC];
    complex<FFT_OUTPUT_T> xk_multi_chan [1][FFT_LENGTH_QC];
    ...
    fft_core<
        ...
        1,
        FFT_DATA_FORMAT
    >(xn_multi_chan, xk_multi_chan, status, config_ch,fftRows);
...
template<
    typename CONFIG_T
>
void fftMultiFramesPipelinedStream(
    complex<float> xn[FFT_LENGTH_QC],
    complex<float> xk[FFT_LENGTH_QC],
    ip_fft::status_t<CONFIG_T>* status,
    ip_fft::config_t<CONFIG_T>* config_ch,
	int fftRows)
{
...
#pragma HLS data_pack variable=xk
    ...
    >(xn, xk, status, config_ch,fftRows);
} // End of 1-channel, floating-point

} // namespace hls
#endif // __cplusplus
#endif // HLS_FFT_PIPELINED_STREAM_HPP
